...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        # crossAttn = model.model.condCrossAtt
        posemb = model.model.posEmbedding
        x_pos_cond = posemb(model_kwargs['poslist'].to(x.device))
        # th.save(x, 'x.pt')
        # th.save(x_pos_cond, 'x_pos_cond.pt')
        # x_pos_cond = crossAttn(x, x_pos_cond)
        # x_cond = x_pos_cond + x
        # th.save(x_cond, 'x_cond.pt')
    ...
    def training_losses_e2e(self, model, x_start, t, img, model_kwargs=None, noise=None):
        ...
        np.random.seed(1230)
        learnable_param_list = np.random.choice([True, False], size=128, p=[0.15, 0.85])
        learnable_param_index = np.where(learnable_param_list==True)
        pos_start_cond[learnable_param_index] = model.model.module.learnable_weight
