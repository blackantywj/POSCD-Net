...
class TrainLoop:
    ...
    def forward_only(self, batch, cond):
        with th.no_grad():
            ...
            for i in range(0, batch.shape[0], self.microbatch):
                ...
                img = micro_cond['img']
                micro_cond['img'] = img.to(th.float32)
                micro_cond['pos'] = micro_cond['pos'][:, :30]
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            # print(batch.shape[0], self.microbatch)
            micro = batch[i : i + self.microbatch].to(dist_util.dev())
            micro_cond = {
                k: v[i : i + self.microbatch].to(dist_util.dev())
                for k, v in cond.items()
            ...
            # r = min(0.1 * math.tanh(self.step/30000-1) + 0.1, 0.1)
            losscap = (losses["loss"] * weights).mean()
            losspos = (losses['posloss'] * weights).mean()
            log_loss_dict(
                self.diffusion, t, {k: v * weights for k, v in losses.items()}
            )
            loss = losscap + losspos
            if self.use_fp16:
