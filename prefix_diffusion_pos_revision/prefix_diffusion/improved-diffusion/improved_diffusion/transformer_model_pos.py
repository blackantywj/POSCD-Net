...
class TransformerNetModel2(nn.Module):
    ...
    def __init__(
        ...
        self.posEmbedding = nn.Embedding(14, 48)
    ...
    def forward(self, x, timesteps, img=None, y=None, att_feats = None, att_masks = None, pos=None, pos_t = None,src_ids=None, src_mask=None):
        """
        Apply the model to an input batch.
        ...
        h = self.output_down_proj(input_trans_hidden_states[: , 30:61])
        h = h.type(x.dtype)
    ...
    def posforward(self, x, timesteps, img=None, y=None, pos=None, src_ids=None, src_mask=None):
        """
           Apply the model to an input batch.
        ...
        posemb = None
        pos_mask = th.ones(img.size(0), 1, 30).cuda()
        ...
        h = h.type(x.dtype)
        mask_token_ids = th.zeros(20, 24).cuda()

        posla = self.posEmbedding(mask_token_ids.to(th.int32), posla)

        outpos = self.posDecode(posla.expand_as(emb_x), img_inputs.detach(), None, pos_mask)

        outpos = self.generator(outpos)

        outputpos = th.max(outpos, dim=-1)
