    ...
    for i,img in enumerate(imgs):
        ...
        for j, s in enumerate(img['sentences']):
            ...
            for i in range(len(s['tokens'])):
                category = nltk.pos_tag([s['tokens'][i]])[0][1]
            ...
            captioneos = caption + [2]
            poseos = pos + [2]
            captionlen = len(caption)
            tmp = (30-captionlen)*[0]
            caption = caption + tmp
            captions.append(caption)
            captioneoslen = len(captioneos)
            tmp = (31-captioneoslen)*[0]
            captioneos = captioneos + tmp
            caption_w_eos.append(captioneos)
            poslen = len(pos)
            tmp = (30 - poslen) * [0]
            pos = pos + tmp
            poss.append(pos)
            poseoslen = len(poseos)
            tmp = (31 - poseoslen) * [0]
            poseos = poseos + tmp
            pos_w_eos.append(poseos)
        while(len(captions)!=5):
            if len(captions)< 5 :
                captions.append(captions[-1])
                poss.append(pos[-1])
                caption_w_eos.append(caption_w_eos[-1])
                pos_w_eos.append(pos_w_eos[-1])
            if len(captions)> 5 :
                captions = captions[:5]
                poss = poss[:5]
                caption_w_eos = caption_w_eos[:5]
                pos_w_eos = pos_w_eos[:5]
        caption_w_eos = np.array( caption_w_eos)
        pos_w_eos =np.array( pos_w_eos)
        poss = np.array(poss)
        captions = np.array(captions)

    json.dump(imgs, open(params['output_json'], 'w'))
...
if __name__ == "__main__":
    ...
    # input json
    # parser.add_argument('--input_json', default='E:\py program\self-critical.pytorch-master\scripts\data.json')
    parser.add_argument('--input_json', default='E:\py program\self-critical.pytorch-master\captioning\data\dataset_coco.json')
